    ...
    private void registerE_btn1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registerE_btn1ActionPerformed
        ...
        try{
            ...
            mgr_stmt.executeUpdate();
                new_id  = db.createStatement().executeQuery("SELECT max(eid) from employee;");
                new_id.next();
                int mg_id = new_id.getInt("max(eid)");
                mgr_stmt = db.prepareStatement("UPDATE branch SET mgr_id = " + mg_id + " WHERE ISNULL(mgr_id);");
                mgr_stmt.executeUpdate();
